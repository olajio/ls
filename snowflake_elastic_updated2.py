...
def load_private_key_from_file(keypair_path, passphrase=None):
    ...
    Returns:
        Private key in DER format as bytes (required by Snowflake)
    """
    try:
        ...
        # Convert to DER format (bytes) as required by Snowflake
        ...
        logger.info(f"Successfully loaded and converted private key from {keypair_path}")
...
def fetch_snowflake_data(user, keypair_path, account, warehouse, sql_query, passphrase=None):
    ...
    try:
        # Load the private key from file (returns DER bytes)
        private_key_der = load_private_key_from_file(keypair_path, passphrase)
        ...
        connection = snow.connect(
            account=account,
            user=user,
            private_key=private_key_der,  # Use DER-formatted private key bytes
            warehouse=warehouse,
            database="SNOWFLAKE"
        )
